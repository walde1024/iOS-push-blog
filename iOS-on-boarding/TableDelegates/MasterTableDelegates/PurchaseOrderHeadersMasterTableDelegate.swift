//
// PurchaseOrderHeadersMasterTableDelegate.swift
// iOS-on-boarding
//
...
class PurchaseOrderHeadersMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [PurchaseOrderHeader] = [PurchaseOrderHeader]()
    var entities: [EntityValue] {
        get {
            return _entities
        }
        set {
            self._entities = newValue as! [PurchaseOrderHeader]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> ()) {
        self.dataAccess.loadPurchaseOrderHeaders() { (purchaseorderheaders, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let purchaseorderheader = self.entities[indexPath.row] as! PurchaseOrderHeader
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "PurchaseOrderId", value: "\(purchaseorderheader.purchaseOrderID)")
        return cell
    }
