//
// StockMasterTableDelegate.swift
// iOS-on-boarding
//
...
class StockMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [Stock] = [Stock]()
    var entities: [EntityValue] {
        get {
            return _entities
        }
        set {
            self._entities = newValue as! [Stock]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> ()) {
        self.dataAccess.loadStock() { (stock, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let stock = self.entities[indexPath.row] as! Stock
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductId", value: "\(stock.productID)")
        return cell
    }
