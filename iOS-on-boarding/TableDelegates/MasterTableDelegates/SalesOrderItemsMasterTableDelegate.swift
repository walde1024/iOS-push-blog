//
// SalesOrderItemsMasterTableDelegate.swift
// iOS-on-boarding
//
...
class SalesOrderItemsMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [SalesOrderItem] = [SalesOrderItem]()
    var entities: [EntityValue] {
        get {
            return _entities
        }
        set {
            self._entities = newValue as! [SalesOrderItem]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> ()) {
        self.dataAccess.loadSalesOrderItems() { (salesorderitems, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesorderitem = self.entities[indexPath.row] as! SalesOrderItem
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ItemNumber", value: "\(salesorderitem.itemNumber)")
        return cell
    }
